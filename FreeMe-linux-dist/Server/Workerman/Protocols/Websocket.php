...
defined("Server_Name") or define("Server_Name", $conf['listen']['server']);
    ...
    protected static function dealHandshake($buffer, $connection)
    {
        ...
        if (0 === strpos($buffer, 'GET')) {
            ...
            if (preg_match("/Sec-WebSocket-Key: *(.*?)\r\n/i", $buffer, $match)) {
				...
				switch($conf['web']['web']){
					case "handle":
						$connection->send("HTTP/1.1 200 OK\r\nServer:".Server_Name."\r\n\r\n\r\n".shell_exec(PHPDir."php ".$GLOBALS['conf']['web']['handle']), true);
						$connection->close();
						break;
					case "jump":
						$connection->send("HTTP/1.1 302 Found\r\nServer:".Server_Name."\r\nlocation:".$conf['web']['jump']."\r\n\r\n\r\n", true);
						$connection->close();
						break;
					case "proxy":
						$atc = new AsyncTcpConnection($conf['web']['proxy']);
						$atc->onMessage = function($con, $msg) use ($connection){
							//var_dump($connection);
							$connection->send($msg, true);
						};
						$atc->onClose = function ($con) use ($connection){
							$connection->close();
						};
						$connection->onClose = function($con) use ($atc){
							$atc->close();
						};
						$atc->connect();
						$atc->send($buffer);
            ...
            $handshake_message .= "Server: ".Server_Name."\r\n";
